...
async function triggerSaveAndClose(page: Page) {
  const actions = page.locator('#invoice-header-actions');
  const saveCloseButton = actions.getByRole('button', { name: /^save\s*&\s*close$/i }).first();
  await saveCloseButton.scrollIntoViewIfNeeded().catch(() => {});
  await expect(saveCloseButton).toBeVisible({ timeout: 10000 });
  await expect(saveCloseButton).toBeEnabled({ timeout: 5000 });

  const trySelectSaveAndClose = async () => {
    await saveCloseButton.click();
    const menu = page.getByRole('menu').first();
    await expect(menu).toBeVisible({ timeout: 5000 });
    const menuItem = menu.getByRole('menuitem', { name: /^save\s*&\s*close$/i }).first();
    await expect(menuItem).toBeVisible({ timeout: 5000 });
    await menuItem.hover();
    await expect(menuItem).toHaveAttribute('data-highlighted', '', { timeout: 3000 });
    await menuItem.click();
  };

  const saveRequest = page.waitForResponse(
    (resp) => resp.request().method() === 'POST' && resp.url().toLowerCase().includes('invoice'),
    { timeout: 20000 },
  );

  try {
    await trySelectSaveAndClose();
    await saveRequest;
  } catch {
    // Retry once using keyboard selection
    await saveCloseButton.click().catch(() => {});
    await page.keyboard.press('ArrowDown');
    await page.keyboard.press('Enter');
    await saveRequest;
  }

  await page.waitForURL(/\/sales\/invoices(?!\/new)/, { timeout: 20000 }).catch(() => {});
  return true;
}
...
async function fillQtyPriceIfEmpty(row: Locator, _index: number) {
  const inputs = row
    .locator('input, [role="spinbutton"]')
    .filter({ hasNot: row.locator('input[type="hidden"]') });
  await expect(inputs.first()).toBeVisible({ timeout: 5000 });
  await expect(inputs.nth(1)).toBeVisible({ timeout: 5000 });

  const qtyInput = inputs.nth(0);
